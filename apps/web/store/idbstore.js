...
function IdbKvStore(name, opts, cb) {
  ...
  function onNewListener(event) {
    ...
    if (!self._channel)
      return self.emit("error", new Error("No BroadcastChannel support"));
  }
...
Transaction.prototype.add = function (key, value, cb) {
  ...
  if (typeof value === "function" || (value == null && cb == null))
    return self.add(undefined, key, value);
  ...
  self._getObjectStore(function (err, objectStore) {
    ...
    try {
      var request =
        key == null ? objectStore.add(value) : objectStore.add(value, key);
    } catch (e) {
      return cb(e);
    }
...
Transaction.prototype.count = function (range, cb) {
  ...
  self._getObjectStore(function (err, objectStore) {
    ...
    try {
      var request =
        range == null ? objectStore.count() : objectStore.count(range);
    } catch (e) {
      return cb(e);
    }
